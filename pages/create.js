...
export default function CreateProjectPage(props) {
  ...
  // const branchUrl = `${API_URL}/brnaches?populate=*`;
  ...
  const onSubmit = async (e) => {
    ...
    if (!res.ok) {
      ...
      // router.push("/");
      props.setModalClass("");
      props.setModalText("Project successfully created!");
  ...
  if ((!pmsError && !pms) || (!areasError && !areas)) {
    ...
    return <div>{pmsError.message}</div>;
  } else {
    return (
      <>
        <Header props={props} />
          ...
          <div className="mb-6">
            ...
            >
              ...
              {/* {values.related_area &&
                Object.values(
                  areas.data[
                    values.related_area ? Number(values.related_area) - 1 : 0
                  ].attributes.branches.data
                ).map((branch) => (
